...
{
	$user_id = select_single_col('we_users_event_guestlist','event_user','WHERE id='.$_POST['row_id']);
	if($user_id == $_SESSION['reg_id'])
	{
		$delete_id = delete_data('we_users_event_guestlist','WHERE id='.$_POST['row_id']);
		if($delete_id > 0)
		{
